...
config_socat(){
    echo -e "${Green}请输入Socat配置信息！如有不懂请联系QQ:5860644 微信:Lyjsolo 邮箱:Lyjsolo@qq.com${Font}"
...
systemctl start rc-local >/dev/null 2>&1
    ...
    sleep 3
    echo
    echo -e "${Green}安装并配置成功!如有不懂请联系QQ:5860644 微信:Lyjsolo 邮箱:Lyjsolo@qq.com${Font}"
...
status_socat(){
    if [ -s /usr/bin/socat ]; then
    echo -e "${Green}检测到Socat已存在，并跳过安装步骤！如有不懂请联系QQ:5860644 微信:Lyjsolo 邮箱:Lyjsolo@qq.com${Font}"
        main_x
    else
        main_y
    fi
